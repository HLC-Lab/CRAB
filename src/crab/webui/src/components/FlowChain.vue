...
<template>
  <div class="chain">
    <div class="node" :style="{ borderLeftColor: node.color || 'var(--border2)' }">
      <span v-if="node.measured" class="meas-badge" title="Metrics are collected for this app">M</span>
      ...
      <span v-if="node.group || node.nodes != null || node.note" class="node-foot">
        ...
        <span v-if="node.nodes != null">{{ node.nodes }} node{{ node.nodes === 1 ? "" : "s" }}</span>
...
.node { position: relative; min-width: 10rem; border-radius: var(--r); padding: 0.45rem 0.6rem;
  border: 1px solid var(--border2); border-left-width: 3px; border-left-style: solid;
  display: flex; flex-direction: column; gap: 0.25rem; background: var(--bg1); }
.node-name { font-size: 0.82rem; color: var(--text); word-break: break-all; padding-right: 1.1rem; }
.node-foot { font-size: 0.68rem; color: var(--text3); display: flex; flex-wrap: wrap; gap: 0.25rem; }
.meas-badge { position: absolute; top: 0.4rem; right: 0.5rem; width: 14px; height: 14px;
  border-radius: 50%; background: var(--ok); color: #06210f; font-size: 0.55rem; font-weight: 700;
